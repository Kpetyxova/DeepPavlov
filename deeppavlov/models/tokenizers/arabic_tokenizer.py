...
@register('stream_arabic_tokenizer')
class StreamArabicTokenizer(Component):
    ...
    def __init__(self, disable: Optional[Iterable[str]] = None, filter_stopwords: bool = False,
        ...
        self.tokenizer = getattr(nltk.tokenize, "wordpunct_tokenize", None)
    ...
    def _lemmatize(self, data: List[str], ngram_range: Optional[Tuple[int, int]] = None, batch_size: int = 10000,
       ...
       Yields:
        ...
        for i, doc in enumerate(data):
            lemmas = self.lemmatizer.lemmatize_text(doc)
            if _lowercase:
    ...
    def _filter(self, items: List[str], alphas_only: bool = True) -> List[str]:
        ...
        if _alphas_only:
            filter_fn = lambda x: x.isalpha() and not x.isspace() and x not in self.sw
        else:
            filter_fn = lambda x: not x.isspace() and x not in self.sw
